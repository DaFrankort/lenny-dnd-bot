...
class CharacterGen:
    ...
    def __init__(self, itr: discord.Interaction, data: DNDData):
        ...
        self.character_class = random.choice(data.classes.entries)
    ...
    def sort_stats_for_class(
        ...
        for table in data.tables.get(
            query="Assign Ability Scores; Standard Array by Class"
        ):
            if (
                r"#assign%20ability%20scores%3b%20standard%20array%20by%20class_xphb"
                in table.url
            ):
                ability_table = table
                break
        ...
        stats = [score for _, score in self.stats_class]
...
class CharacterGenEmbed(SimpleEmbed):
    def __init__(self, character: CharacterGen):
        ...
        self.add_field(
            name="",
            value=f"**Background**: {character.background.name.title()}",
            inline=False,
        )
